...
export default function Header(props: any) {
    ...
    const delTrace = (index: number) => {
        ...
        for (var i = 0; i < trace.length; i++) {
            if (index != i) {
                temp_trace.push(trace[i]);
            }
        }
        trace = temp_trace;
        sessionStorage.setItem('trace', JSON.stringify(temp_trace));
        // setTraces && setTraces(temp_trace && []);
        setTraces(temp_trace);
    };
