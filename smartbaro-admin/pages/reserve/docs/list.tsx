...
const ReserveList: NextPage = (props: any) => {
    ...
    const getPostsData = async p => {
        try {
            const { data } = await api.post(`/be/admin/reserve/docs/list`, p);
            setParams(param => {
    ...
    const getFilterContidion = async () => {
        try {
            const { data } = await api.post(`/be/admin/reserve/docs/filter`);
            setFilter(data);
        } catch (e: any) {}
    };
    ...
    const fnEditState = async () => {
        try {
            ...
            const { data } = await api.post(`/be/admin/reserve/docs_state/edit`, p);
    ...
    const goEdit = (item: any) => {
        window.open(`/reserve/docs/edit?uid=${item.uid}`, '제증명서류발급신청 상세', 'width=1120,height=800,location=no,status=no,scrollbars=yes');
    };

    return (
        <Layout user={props.user} title="국제바로병원 인트라넷" nav_id={35} crumbs={['병원관리', '제증명서류발급 신청내역']}>
            <form onSubmit={fn.handleSubmit} noValidate className="w-full border py-4 px-6 rounded shadow-md bg-white mt-5">
                <div className="grid grid-cols-4 gap-6">
                    ...
                    <div className="col-span-1">
                        <label className="form-label">발급희망일 조회</label>
                        <Datepicker
                            containerClassName="relative w-full text-gray-700 border border-gray-300 rounded"
                            inputName="hope_at"
                            value={s.values.hope_at}
                            i18n={'ko'}
                            onChange={fn.handleChangeDateRange}
                            primaryColor={'blue'}
                        />
            ...
            <div className="col-table-header">
                ...
                <div className="text-right flex items-center">
                    <div className="font-bold me-3">선택한 신청내역을</div>
            ...
            <div className="col-table">
                <div className="col-table-th grid grid-cols-10 sticky top-16 bg-gray-100">
                    ...
                    <div className="">신청인</div>
                    <div className="col-span-2">신청인 연락처</div>
                    <div className="">발급희망일</div>
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/admin/reserve/docs/list`, request);
        response = data;
